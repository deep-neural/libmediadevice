...
int main() {
    // Create audio device configuration for WASAPI
    media::AudioDeviceConfig config;
    config.type = media::AudioDeviceType::WASAPI;
    ...
    if (!audio_device) {
        std::cerr << "Failed to create WASAPI audio device!" << std::endl;
        return 1;
    }
    
    std::cout << "Successfully created WASAPI audio device" << std::endl;
...
int main() {
    std::cerr << "WASAPI audio device is only available on Windows platforms." << std::endl;
    return 1;
}
